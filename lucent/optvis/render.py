...
from lucent.optvis import objectives, transform, param, hooks
    ...
    try:
        for i in tqdm(range(1, max(thresholds) + 1), disable=(not progress)):
            def closure():
                ...
                loss = objective_f(model_hook.get_hook())
                loss.backward()
                return loss
            ...
            if i in thresholds:
                ...
                if verbose:
                    print("Loss at step {}: {:.3f}".format(i, objective_f(model_hook.get_hook())))
                    if show_inline:
                        show(image)
                images.append(image)
...
def view(tensor):
    ...
    assert len(image.shape) in [
        2, 3, 4,
    ], "Image should have 2, 3, or 4 dimensions, invalid image shape {}".format(image.shape)
